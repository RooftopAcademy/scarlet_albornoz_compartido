export default function cartProduct(p) {
    ...
    <li class="cart-list-item d-flex j-space-between">
        ...
        <span class="item-qty">
                <button class="btn js-add-1" data-product-id="${p.id}">+</button>
                 ${p.qty} 
                <button class="btn js-remove-1" data-product-id="${p.id}">-</button>
                </span>
            <button class="btn js-remove-from-cart btn-remove" data-product-id="${p.id}">
